...
express.Router() is used to make the process of route handling easier and cleaner for devs. It lets you split handlers into different files (if necessary), allows cleaner middleware implementation, etc. Javascript can get very cluttered especially with larger applications, express allows for code to be cleaner, more reusable, maintanable, and expandable.
...
as shown in the last question of section A, the following code is used:
app.use((err,req,res,next) => {
...
the app.listen() method is used to "tell" express to begin listening for incoming requests. the process.env.port is an environment variable containing the default port of the machine, or one of the dev's choice without needing it to be specified in every JS file. It is followed by || 8081, which is just there to be a default port if the environment port is not set. The environment variable is especially useful on production environment where the port may be dynamic.
---
